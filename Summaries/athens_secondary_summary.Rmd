...
Three basic models will be presented here. Each has been calibrated to fit the population size, date of the first confirmed case, and likely number of exposed (but not infectious) and infectious persons in the community when that first case was reported. A natural epidemic model will be included as the absolute worst-case scenario, with the basic assumption that the social distancing intervention put into place by Athens-Clarke and Oconnee counties has little to no effect on the spread of the virus. The other two models are calibrated to reduce transmission by 25% (average) and 50% (excellent). 50% is the estimated effect of very effective social distancing (and essentially lockdown) as was seen in some other settings.

The models are limited to showing results over the next two weeks.


### The Natural Epidemic Model


```{r, include=FALSE} 
epid_mod <- read_rds(paste0("../Models/epidemic_base_", latest_date))
epid_hosp_mod <- read_rds(paste0("../Models/epidemic_base_hosp_", latest_date, ".rds"))
```
...
```{r, include=FALSE} 
avg_mod <- read_rds(paste0("../Models/social_distance_upper_", latest_date))
avg_hosp_mod <- read_rds(paste0("../Models/social_distance_upper_hosp_", latest_date, ".rds"))
```
...
The total hospitalization count ends up roughly half of what would be expected with the natural epidemic.
...
The relative slowing down of the flow of new patients also slows down the number of new hospitalizations which eventually is outweighed by the number of discharges and the current number of patients hospitalized begins to drop in early April after peaking near 30.
...
```{r, include=FALSE} 
exc_mod <- read_rds(paste0("../Models/social_distance_base_", latest_date))
exc_hosp_mod <- read_rds(paste0("../Models/social_distance_base_hosp_", latest_date, ".rds"))
```
...
The number of new patients levels off to near zero and there are only sporadic new cases.
...
With excellent social distacncing, over the next two weeks the number of hospitalized patients begins to drop in the next few days after peaking at near 25.
