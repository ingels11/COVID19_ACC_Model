...
data_date <- as.Date("2020-04-08")
datatwo <- data[1:which(data$date == data_date), ]

datatwo$total <- rowSums(datatwo[,c(2:18)])
...
write_rds(outBaselineInt, 
          paste0("Models/", "poor_model_", Sys.Date(), ".rds"))
...
plot.model.acc(outSD,  dailyCases$date[1:which(dailyCases$date == Sys.Date())], 
               dailyCases2$secondary_cum[1:which(dailyCases$date == Sys.Date())],
               log='y', title='Excellent Social Distancing Model')

write_rds(outSD, paste0("Models/", "exc_model_", Sys.Date(), ".rds"))
...
plot.model.acc(outSDUpper, dailyCases$date[1:which(dailyCases$date == Sys.Date())], 
               dailyCases2$secondary_cum[1:which(dailyCases$date == Sys.Date())], 
               log='y', title='Average Social Distancing Model')


write_rds(outSDUpper, paste0("Models/", "avg_model_", Sys.Date(), ".rds"))
